...
export class UserAdmin extends Component {
  ...
  refreshList() {
    ...
    if (this.state.orderBy)
      apiLink += `&OrderBy=${this.state.orderBy}&Order=${this.state.order}`;
    fetch(apiLink, {
      method: "GET",
  ...
  onOrderChange(ev) {
    ...
    if (this.state.orderBy != ev.target.getAttribute("name"))
      this.setState({ orderBy: ev.target.getAttribute("name") }, () =>
        this.refreshList()
      );
    if (this.state.orderBy == ev.target.getAttribute("name"))
      this.setState(
            ...
            <Table className="mt-2" striped bordered hover size="sm">
              <thead>
                <tr className="text-center">
                  <th name="Id" onClick={this.onOrderChange}>
                    User Id <i className="fa-solid fa-arrow-up-arrow-down"></i>
                  </th>
                  <th name="Name" onClick={this.onOrderChange}>
                    Name <i className="fa-solid fa-arrow-up-arrow-down"></i>
                  </th>
                  <th>Email Id</th>
                  <th>Active From</th>
                  <th>Role</th>
