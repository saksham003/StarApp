    ...
    public class EmployeeRepository: GenericRepository<Employee>, IEmployeeRepository
    {
        ...
        public async Task<EmployeePageDto> GetEmployees(EmployeeFilter filter)
        {
            ...
            if (filter.OrderBy != null)
            {
                ...
                {
                    employees =filter.Order == "ASC"? employees.OrderBy(x => x.Id): employees.OrderByDescending(x => x.Id);
                }
                if (filter.OrderBy == "Name")
                {
                    employees =filter.Order == "ASC"? employees.OrderBy(x => x.UserName): employees.OrderByDescending(x => x.UserName);
                }
            }
            if (filter.OrderBy == null)
            {
        ...
        public async Task PostEmployee(PostEmployeesDto employeeDto)
        {
